...
- **Fabric ID** — a shared, non-secret identifier (YAML constant) that gates forwarding/membership so all VasoSmart pots relay for each other (the "unified mesh"). No security role.
...
Routing-visible header fields (Fabric ID, src/dst/next-hop hashes, msg_id, TTL) and HELLO/routing stay **cleartext** so relays without the group key can still forward. Only payloads are protected. Nodes start **unprovisioned** (no key → inert) until paired.
